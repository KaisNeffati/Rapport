...
\initial{A}fter defining the project scope and making a preliminary study. In this chapter, we will deep dive in two
...
\label{sec:sec01}   
 ...
 \begin{figure}[H]
	...
	\mycaption[Promotion Use Case Diagram.]{Promotion Use Case Diagram.}
	\label{fig:FilialesEtClients}
\end{figure}
...
A great solution for this problem could be a messaging layer that gives a various guarantees 
...
\begin{figure}[h!]
	...
	\mycaption[AWS SQS benshMark.]{ASW SQS benshMark . Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
\begin{figure}[h!]
	...
	\mycaption[Kafka perfomance benshMark.]{Kafka perfomance benshMark. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
The maximum send throughput with multiple thread/nodes is about 10 500 msgs/s,
 ...
 \begin{figure}[h!]
	...
	\mycaption[Mongodb message queue benshMark.]{Mongodb message queue benshMark. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
With 25 threads, we are up to 12 800 msgs/s! And finally, with 25 threads and 4 nodes, we can achieve 17 000 msgs/s.
 \begin{figure}[h!]
	...
	\mycaption[HornetQ benshMark.]{HornetQ benshmark. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
\paragraph{RabbitMQ}
\label{sec:sec01}
...
This scales nicely as we add nodes, up to 1 600 msgs/s
 \begin{figure}[h!]
	...
	\mycaption[RabbitMQ benchmark. ]{RabbitMQ benchmark. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
The next benchmark show the perfomance of ActiveMQ  :
 \begin{figure}[h!]
	...
	\mycaption[ActiveMQ benchmark.]{ActiveMQ benchmark. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
When looking only at the throughput, Kafka is a clear winner
 (unless we include SQS with multiple nodes, but as mentioned, that would be unfair)
  \begin{figure}[h!]
	...
	\mycaption[kafka benshmark.]{kafka benshmark. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}\\
It's also interesting to see how sending more messages in a batch improves the throughput, when increasing the batch size from 10 to 100, Rabbit gets a 2x speedup, HornetQ a 1.2x speedup, and Kafka a 2.5x speedup, achieving about 89 000 msgs/s!
  \begin{figure}[h!]
	...
	\mycaption[stream message queues comparative.]{stream message queues comparative. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}\\
...
one depending on some benshMarks well known.
 \begin{figure}[h!]
	...
	\mycaption[NoSql benshmark comparative.]{NoSql benshmark comparative. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
\begin{figure}[h!]
	...
	\mycaption[Polyglot Persistance in practice..]{Polyglot Persistance in practice. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
\begin{figure}[h!]
	...
	\mycaption[Polyglot Persistance.]{Polyglot Persistance. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
\begin{figure}[h!]
	...
	\mycaption[BigData Processing frameworks.]{BigData Processing frameworks. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
...
\begin{figure}[h!]
	...
	\mycaption[API Gatway.]{API Gatway. Figure taken from \cite{SofrecomWebsite}.}
	\label{fig:FilialesEtClients}
\end{figure}
